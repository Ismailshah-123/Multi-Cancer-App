...
def handle_answer_tts(ans, lang_choice="English", tenant_id="guest"):
    ...
    if lang_choice.lower() == "urdu":
        urdu_text = tenant_cache.get(ans)
        if not urdu_text:
            urdu_text = GoogleTranslator(source="en", target="ur").translate(ans)
            tenant_cache[ans] = urdu_text

        st.info("🔊 Playing Urdu audio...")
        speak_text_streamlit(urdu_text, lang="ur")

    else:
        st.info("🔊 Playing English audio...")
        speak_text_streamlit(ans, lang="en")
...
# ---------------- MODELS ----------------
HF_REPO_ID = "Ismailshah/multi-cancer-models"

MODEL_FILES = {
    "BCN": "bcn_resnet50_best.pth",
    "BRAIN": "best_brain_model3.pth",
    "GASTRIC": "best_gastric_model_optimized.pth",
    "BREAST": "breakhis_resnet50_best.pth",
    "PADUFES": "padufes_resnet50_best.pth",
    "HAM10000": "ham10000_resnet50_best.pth",
    "LUNG": "best_lung_model.pth",
    "OVARIAN": "ovarian_resnet50_best.pth",
    "GENERALIST": "generalist_resnet50_best.pth",
}
...
# ---------- PDF REPORT ----------
    ...
    with tabs[1]:
        ...
        # Single text input with unique key per session
        ...
        # Voice input button
            if st.button(
               ...
               try:
                  ...
                  q_input = recognizer.recognize_google(audio)
                  st.success(f"🎙 You said: {q_input}")
               except Exception as e:
